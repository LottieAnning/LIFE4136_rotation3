...
suball= []
for file in inall:
    # if str(search+args.ci+'sd_') in file:   changed this line to search in life, group me
    if search in file:
        suball.append(file)
suball.sort()
...
# manaually changed by group from is to == due to errors
if args.out == 'na':
    for file in suball:
        if file.endswith(str(int(args.ovlp))+'ol.txt'):
            genelist.append(file)
else:
    for file in suball:
        if file.endswith(args.out+'_'+str(int(args.ovlp))+'ol.txt'):
            genelist.append(file)
genelist.sort()
...
# set a list of lyrata genes that we are interested in and figures will only be produced for those genes in the list
meiosis_genes = {"AL1G50380":'"FANCONI ANEMIA COMPLEMENTATION GROUP M (FANCM)"',
                 "AL1G48910":'"PARALLEL SPINDLE 1 (PS1)"',
                 "AL1G21590":'"POOR HOMOLOGOUS SYNAPSIS 1 (PHS1)"',
                 "AL1G16670":'"JASON (JASON)"',
                 "AL1G26770":'"SOLO DANCERS (SDS)"',
                 "AL1G22040":'"WINGS APART-LIKE PROTEIN 1 (WAPL1)"'}

for file in genelist:
    tbasename = file.replace(contrast+'_','')
    basename = tbasename.replace('ol.txt','')
    ...
    for line in inputfile:
        ...
        # change the name of the scaffold the R script looks for in the the lyrata genes out file. Edited by Sam and D
        if scaf == 'scaffold_1':
            scaf = 'NW_003302555.1'

        # The actual value in tline is very weird and it has quotations as part of the actual string so this removes that and allows you to check for the gene in our meiosis gene list. Edited by Sam and D
        if tline.replace('"','') in meiosis_genes.keys():
            # changed line 189 to give plot title as the actual gene name and not the lyrata gene name
        
            for i in range(len(allsites)):
                print('\nCreating overview plot for candidate gene\n\n\t'+ttline+'\n\nfrom list '+str(inputdir+file)+'\nin '+outputdir) 
                # rfile = open(outputdir+final_output_name+basename+args.suffix+'.r', 'w')
                rfile = open(outputdir+ttline+basename+args.suffix+'.r', 'w')
                rfile.write('# load libraries\n'+
                            'library(ggplot2)\n'+
                            'library(grid)\n'+
                            'library(grid)\n'+
                            # edited read table 139 to include sep = \t
                            'f <- read.table("'+args.geneor+'",header=F,sep="\\t")\n'+
                            'locus='+tline+'\n'+
                            ...
                            # change scaffold='scaffold_' with NW00.1 because our files have that name instead of scaffold 1 because we had to change it for g1 to run 
                            's25 <- subset(ts25[ts25$scaffold=="NW_003302555.'+scaf+'", ])\n'+
                            ...
                            '   ggtitle('+meiosis_genes[tline.replace('"','')]+') +\n'+
                ...
                '   geom_line(color=I("green")) +\n'+
                            ...
                            '   ends=genes[which(genes[,1]==locus),"geneor"]))\n\n'+
                            'pdf("'+outputdir+ttline+'_'+basename+'_'+str(winwidth)+'kb'+args.suffix+'.pdf", width=8, height=5)\n'+
                            'grid.newpage()\n'+
                            ...
                            'print(p.AFDs)\n'+ # remove this line and uncomment all the lines around it for original output. changed by S and D
                            # 'print(p.AFDs, vp=vplayout(1:2,1))\n'+  # edited out all these lines so that only the AFD plot is create
                ...
                # cmd = ('Rscript '+outputdir+final_output_name+basename+args.suffix+'.r')
